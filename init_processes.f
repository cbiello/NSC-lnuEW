...
c     set the bornzerodamp flag to true by default (DECIDE IF YOU WANT THIS FOR THIS PROCESS)
...
! Coupling order: order_ew -> NLO QCD
      call set_parameter("order_qcd", res_powst)
...
! select EW input scheme: MZ, MW, alpha (ew_scheme=2, default) VS. MZ, MW, Gmu (ew_scheme=1)
      ...
      call set_parameter("psp_tolerance",0.000001)
...
c     set the number of final-state paricles at the Born-level WITHOUT any resonance
        ...
        if(powheginput("chooseDY").gt.0.1) then

           print*, '************* DY PROCESS ***************'
           print*, '******* Positive Charged DY (W+) *******'
