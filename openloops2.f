...
! Register & Loopup processes
        function register(flav,amptype,nflav,intid)
...
!     alpha_QCD=st_alpha
!     DL: added in order for the first born amplitude not to be zero:
!     OL initialize alpha strong only after the second call to the born
!     -> for processes with 1 jet, alpha_s should be immediatly initialized
          
          gQCD=sqrt(4*pi*st_alpha)
c     call set_parameter("alphas", st_alpha) !alpha_qcd
...
!            call evaluate_ccmatrix(id, p, born, bornjk, bornew)
            ...
            ! VS: this sign is explicitly corrected for elsewhere, 
            ! changing it carelessly might break something, use caution
            bornjk = -1*bornjk
          end if
...
! Color flow
      subroutine openloops_colour(p,flav,color,ifl)
        ...
        external random
        id = register(flav,1,intid=intid)
...
! Physical parameters
        ...
        if (ph_gfermi  /= 0) call set_parameter("gmu", ph_gfermi)
...
! Select oppenLoops reduction library. Default: COLLIER   --> DL commented it
c$$$      if (powheginput('#olpreset')>0) then
c$$$        call set_parameter("preset", powheginput('#olpreset'))
c$$$      else
c$$$        call set_parameter("preset", 2)
c$$$      end if
...
! Fortran77 Interfaces
      ...
      subroutine openloops_uuborn(p,bflav,born,bornjk,bmunu)
        use openloops_powheg, only: ol_born => openloops_born
        implicit none
        include 'nlegborn.h'
        double precision,intent(in) :: p(0:3,nlegbornexternal-1)
        integer,intent(in) :: bflav(nlegbornexternal-1)
        double precision,intent(out) :: born
        double precision,intent(out),optional :: bornjk(nlegbornexternal-1,nlegbornexternal-1)
        double precision,intent(out),optional :: bmunu(0:3,0:3,nlegbornexternal-1)
        call ol_born(p,bflav,born,bornjk,bmunu)
      end subroutine openloops_uuborn
      ...
      subroutine openloops_uubvirtual(p,vflav,virtual)
        use openloops_powheg, only: ol_virtual => openloops_virtual
        implicit none
        include 'nlegborn.h'
        double precision, intent(in) ::  p(0:3,nlegbornexternal-1)
        integer, intent(in) :: vflav(nlegbornexternal-1)
        double precision, intent(out) ::  virtual
        call ol_virtual(p,vflav,virtual)
      end subroutine openloops_uubvirtual
